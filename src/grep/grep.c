...
void print_matches_only(const char *line, regex_t *regexes, int pattern_count, const GrepFlags *flags,
                        const char *filename, int line_number, int filename_needed) {
    int start = 0;
    int len = strlen(line);
...
// Проверка, соответствует ли строка какому-либо паттерну (для -o с учетом пустых совпадений)
int matches_pattern_for_o(const char *line, regex_t *regexes, int pattern_count, GrepFlags *flags, regmatch_t *pmatch) {
    (void)flags;  // unused parameter
    for (int i = 0; i < pattern_count; i++) {
    ...
    while ((line_len = getline(&line, &line_size, file)) != -1) {
        ...
        if (match) {
            if (flags->flag_c) {
                ...
                print_matches_only(line, regexes, pattern_count, flags, filename, line_number, filename_needed);
            } else {
...
int process_file(const char *filename, regex_t *regexes, int pattern_count, GrepFlags *flags,
                  int filename_needed, int total_file_count) {
...
int handle_e_flag(int argc, char *argv[], int *i, GrepFlags *flags, char patterns[][MAX_PATTERN_LEN], int *pattern_count, int *has_explicit_pattern) {
    (void)flags;
...
int handle_f_flag(int argc, char *argv[], int *i, GrepFlags *flags, char patterns[][MAX_PATTERN_LEN], int *pattern_count, int *has_explicit_pattern) {
    (void)flags;
    if (*i + 1 < argc && argv[*i + 1][0] != '-') {
        (*i)++;
...
int handle_simple_flag(int argc, char *argv[], int *i, GrepFlags *flags, char patterns[][MAX_PATTERN_LEN], int *pattern_count, int *has_explicit_pattern) {
    (void)argc;
    (void)argv;
    (void)i;
    (void)patterns;
    ...
    if (strcmp(argv[*i], "-h") == 0) flags->flag_h = 1;
    else if (strcmp(argv[*i], "-i") == 0) flags->flag_i = 1;
    else if (strcmp(argv[*i], "-v") == 0) flags->flag_v = 1;
    else if (strcmp(argv[*i], "-c") == 0) flags->flag_c = 1;
    else if (strcmp(argv[*i], "-l") == 0) flags->flag_l = 1;
    else if (strcmp(argv[*i], "-n") == 0) flags->flag_n = 1;
    else if (strcmp(argv[*i], "-s") == 0) flags->flag_s = 1;
    else if (strcmp(argv[*i], "-o") == 0) flags->flag_o = 1;
...
static FlagDef flag_defs[] = {
    ...
    {"-f", NULL, handle_f_flag},
    {"-h", NULL, handle_simple_flag},
    {"-i", NULL, handle_simple_flag},
    {"-v", NULL, handle_simple_flag},
    {"-c", NULL, handle_simple_flag},
    {"-l", NULL, handle_simple_flag},
    {"-n", NULL, handle_simple_flag},
    {"-s", NULL, handle_simple_flag},
    {"-o", NULL, handle_simple_flag},
};

// Обработка одного флага по таблице
static int process_flag(int argc, char *argv[], char *arg, int *i, GrepFlags *flags, char patterns[][MAX_PATTERN_LEN],
    ...
    for (size_t j = 0; j < sizeof(flag_defs) / sizeof(flag_defs[0]); j++) {
        if (strcmp(arg, flag_defs[j].short_flag) == 0) {
            return flag_defs[j].handler(argc, argv, i, flags, patterns, pattern_count, has_explicit_pattern);
        }
    }
    return 0;
}
...
int grep_main(int argc, char *argv[]) {
    ...
    while (i < argc) {
        ...
        if (argv[i][0] == '-' && argv[i][1] != '\0') {
            ...
            if (strcmp(argv[i], "-e") == 0 || strcmp(argv[i], "-f") == 0) {
                ...
                if (strlen(argv[i]) > 2) {
                    print_error("grep", "-e and -f cannot be combined with other flags");
                    return 1;
                }
            }
            ...
            if (strlen(argv[i]) == 2) {
                ...
                if (process_flag(argc, argv, argv[i], &i, &flags, patterns, &pattern_count, &has_explicit_pattern) != 0) {
                    return 1;
                }
            } else {
    ...
    for (int j = i; j < argc; j++) {
        int ret = process_file(argv[j], regexes, pattern_count, &flags, file_count > 1, file_count);
        if (ret != 0) {
            has_errors = 1;
        }
        file_found = 1;
    ...
    // Возвращаем код ошибки 2, если были ошибки и флаг -s не установлен
    // (системный grep возвращает 2 при ошибках даже с -s)
    if (has_errors) {
        return 2;
    }
