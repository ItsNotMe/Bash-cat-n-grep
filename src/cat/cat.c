...
int handle_long_flag(int argc, char *argv[], int *i, CatFlags *flags) {
  ...
  if (strcmp(argv[*i], "--number-nonblank") == 0) {
    ...
    return 1;  // сигнал о прерывании (конец флагов)
  }
  return 0;
}
...
static CatFlagDef cat_flag_defs[] = {
    ...
    {NULL, "--number-nonblank", handle_long_flag},
    {NULL, "--number", handle_long_flag},
    {NULL, "--squeeze-blank", handle_long_flag},
    {NULL, "--", handle_end_flags},
};
...
static int process_flag(int argc, char *argv[], char *arg, int *i,
                        CatFlags *flags) {
  for (size_t j = 0; j < sizeof(cat_flag_defs) / sizeof(cat_flag_defs[0]);
       j++) {
    if ((cat_flag_defs[j].short_flag &&
         strcmp(arg, cat_flag_defs[j].short_flag) == 0) ||
        (cat_flag_defs[j].long_flag &&
         strcmp(arg, cat_flag_defs[j].long_flag) == 0)) {
...
int main(int argc, char *argv[]) { return cat_main(argc, argv); }
