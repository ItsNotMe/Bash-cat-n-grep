# Bash-cat-n-grep
Bash utility cat and grep with flags
...
- Не использовать устаревшие и выведенные из употребления конструкции языка и библиотечные функции. Обратить внимание на пометки legacy и obsolete в официальной документации по языку и используемым библиотекам. Ориентироваться на стандарт POSIX.1-2017;
...
- При использовании сторонних библиотек, в Makefile должны быть заложены сценарии сборки, предусматривающие их подключение/загрузку;
...
Проверка реализаций для alpine будет проведена с использованием версии 3.20, где утилиты cat и grep представлены busybox-версиями. Обрати внимание, что поведение busybox может отличаться от GNU-реализаций.
...
- Поддержка _парных_ комбинаций (например, `-iv`, `-in`).
- Поддержка вызова как с флагами, так и без них.
